...
shopt -s nullglob # Prevent globs that match nothing from remaining literal strings
...
LOG_FILE="/var/log/pihole-encryption-setup.log"; SCRIPT_VERSION="1.3.0"
...
create_backup() {
    ...
    print_message "Backup directory: $BACKUP_DIR"; mkdir -p "$BACKUP_DIR"
...
set_toml_value() {
    ...
    cp "$config_file" "$backup_file" # Backup before attempting change
    print_message "  Setting [$section] $key = $value"
    # Use awk for robust TOML editing
...
get_toml_value() {
    ...
    in_section && /^\[/ { in_section = 0; }
    in_section && $0 ~ "^[[:space:]]*" k "[[:space:]]*=" {
        gsub(/^[[:space:]]*|"|'"'"'/, ""); # Remove leading spaces and quotes
        sub(/^[^=]*=[[:space:]]*/, ""); # Remove key and equals sign
        print; exit;
    }' "$config_file"
}
...
verify_pihole_config() {
    ...
    else; print_success "  ✓ Domain correctly set to $DOMAIN"; fi
    # Serve All
    ...
    else; print_success "  ✓ serve_all correctly set to true"; fi
    # Ports (Ensure 80 is present)
    ...
    else; print_success "  ✓ Port 80 is present in configuration"; fi
    # Apply changes
    ...
    else; print_success "✓ Pi-hole configuration is optimal for Let's Encrypt."; fi
    # Show final config
...
fix_webroot_ownership() {
    ...
    echo ""; print_message "Options:"; echo "  1) Change ownership to pihole:pihole (Recommended)"; echo "  2) Keep current owner but set permissions to 755 (Less secure)"; echo "  3) Skip"
    read -p "Choose option (1-3): " owner_choice
    case $owner_choice in
        1) print_message "Changing ownership..."; chown -R "$pihole_user:$pihole_user" "$webroot" 2>/dev/null || sudo chown -R "$pihole_user:$pihole_user" "$webroot" && print_success "✓ Ownership changed." ;;
        2) print_message "Setting permissions to 755..."; chmod -R 755 "$webroot" 2>/dev/null || sudo chmod -R 755 "$webroot" && print_success "✓ Permissions set." ;;
        3) print_warning "Skipping." ;;
    esac
...
verify_port_80_accessible() {
    ...
    local public_ip=""; local test_file="acme-test-$(date +%s).html"; local test_url="http://$DOMAIN/$test_file"
    # Get public IP
    ...
    if curl -s -o /dev/null -w "%{http_code}" "http://127.0.0.1/$test_file" | grep -q "200"; then
        ...
        ls -la "$WEBROOT"; rm -f "$WEBROOT/$test_file"; return 1
    fi
    ...
    rm -f "$WEBROOT/$test_file" # Cleanup
    if [[ "$test_success" == true ]]; then
        ...
        print_warning "Please ensure: 1) Port 80 is forwarded to $LOCAL_IP, 2) Your domain points to $public_ip, 3) No firewall is blocking."
...
generate_self_signed_cert() {
    ...
    chown pihole:pihole "$PIHOLE_CERT" "$PIHOLE_CA_CERT" 2>/dev/null; chmod 600 "$PIHOLE_CERT"; chmod 644 "$PIHOLE_CA_CERT"
...
obtain_letsencrypt_http01() {
    ...
    if ! command -v certbot &> /dev/null; then
        print_message "Installing certbot..."; apt-get update &>/dev/null && apt-get install -y certbot &>/dev/null || { print_error "Failed to install certbot."; return 1; }
    fi
    ...
    if [[ -d "$le_dir" ]]; then
        ...
        if ! certbot certonly --webroot -w "$WEBROOT" --non-interactive --agree-tos --email "$EMAIL" -d "$DOMAIN" >> "$LOG_FILE" 2>&1; then
            print_error "Let's Encrypt challenge failed. Check $LOG_FILE for details."; return 1
        fi
    fi
...
DOMAIN="$DOMAIN"; EMAIL="$EMAIL"; WEB_PORT="$WEB_PORT"; DNS_TLS_PORT="$DNS_TLS_PORT"; LOCAL_IP="$LOCAL_IP"; INSTALL_DATE="$(date)"; SCRIPT_VERSION="$SCRIPT_VERSION"
EOF
...
#!/bin/bash
DOMAIN="\$RENEWED_DOMAINS"; PIHOLE_CERT="$PIHOLE_CERT"; WEBROOT="$WEBROOT"
[ -z "\$DOMAIN" ] && [ -f "$INSTALL_STATE_FILE" ] && source "$INSTALL_STATE_FILE" && DOMAIN="\$DOMAIN"
...
main_install() {
    ...
    if [[ -n "$domain_ip" && "$domain_ip" != "$LOCAL_IP" ]]; then
        print_warning "Domain $DOMAIN resolves to $domain_ip, but server IP is $LOCAL_ALERT_IP."
        read -p "Continue anyway? (y/n): " -n 1 -r; echo; [[ ! $REPLY =~ ^[Yy]$ ]] && exit 0
    fi
    # --- Core Execution Flow ---
...
# --- Menu and Uninstall (Simplified for Master Build) ---
show_menu() {
    clear; echo -e "${GREEN}═══════════════════════════════════════════════════════════════════════════════${NC}"
    echo -e "${GREEN}         Pi-hole Enterprise Encryption Setup v$SCRIPT_VERSION (Master Build)${NC}"
    echo -e "${GREEN}═══════════════════════════════════════════════════════════════════════════════${NC}\n"
    echo "Main Menu:"; echo "  1) Fresh Install"; echo "  2) Uninstall"; echo "  3) Exit"
...
main() {
    case $1 in --uninstall) uninstall; exit 0 ;; --help|--version) echo "Pi-hole Encryption Setup v$SCRIPT_VERSION"; exit 0 ;; esac
    check_root
    show_menu
}
main "$@"